...
## Library (using npm)
Golden Layout shipped via NPM. However, be warned the NPM modules have not been updated in a long time, so [building from source](#source) is currently recommended. If you really want to use the old unsupported modules, use the following commands to install it into an application package:\
`npm i golden-layout`
...
1. (Optional but recommended.) Build and use bundles as [described here](#building-single-file-bundles).
...
Bundles are not built by default and are not included in the NPM package.

If you use EcmaScript modules (recommended), copy either `dist/bundle/esm/golden-layout.min.js` or `dist/bundle/esm/golden-layout.js` (when debugging) into your project as `golden-layout.js`.
Then in your project you can do:
```
import { GoldenLayout, LayoutConfig, ResolvedLayoutConfig, ItemConfig, ComponentItemConfig, ResolvedItemConfig, RowOrColumn, Tab } from './golden-layout.js';
```
